...
dates <- read.delim("../data/InaugurationDates.txt")
#dates
```

#Step 2 - Text processing, pre-processing stage
...
# Get sentences with "women" in it from each speech  
```{r}
...
ui <- fluidPage(
  ...
  h2("Exploratory Analysis of Keywords in Presidential Inauguration Speeches"),
  hr(),
  p("Let's explore the diction of inauguration speeches: What words are popularly chosen? What words are a bit contended? Can we learn anything about a specific inauguration speech and more generally a specific president's term by doing a close analysis of the sentences in which certain keywords are explicitely used? In what context are certain keywords used? And how does the sentiment or emotion behind the sentence that contains a keyword change as the context changes? Can we make any generalizations about word choice in presidential inauguration speeches?"),
  p("Below, enter four keywords you may (or may not) expect to find in a presidential inauguration speech: "),
  
  fluidRow(
    column(3, 
         textInput("term1", label = "Keyword 1", value="", 
                   placeholder="e.g., women")),
    column(3,
         textInput("term2", label = "Keyword 2", value="", 
                   placeholder="e.g., woman")),
    column(3,
         textInput("term3", label = "Keyword 3", value="", 
                   placeholder="e.g., girls")),
    column(3,
         textInput("term4", label = "Keyword 4", value="", 
  ...
  fluidRow(
    h3("Words Associated with Keywords in US Presidential Inauguration Speeches"),
    dataTableOutput("speeches.input")
  ),
  
  fluidRow(
    h3("Table of Speeches that Contain the Keywords, Including Full Text"),
    dataTableOutput("dated.speeches.input.f")
  ),
  
  fluidRow(
    h3("Data from Sentiment Analysis on Sentences Containing Keywords"),
    dataTableOutput("sentence.list.input")
  )
)
...
server <- function(input,output,session){
  ...
  speeches.input <- reactive({get.speeches(tdm.tidy.input())})
  output$speeches.input <- renderDataTable(speeches.input)
  ff.input <- reactive({get.ff.term(folder.path,speeches.input())})
  output$assoc.input <- renderDataTable({assoc.words(folder.path,
                                                      speeches.input(),x=10)})
  dated.speeches.input <- reactive({get.speech.date(speeches.input())})
  dated.speeches.input.f <- reactive({full.text(ff.input(), 
                                        dated.speeches.input())})
  output$dated.speeches.input.f <- renderDataTable(dated.speeches.input.f)
  sentence.list.f <- reactive({get.sentences(dated.speeches.input.f)})
  sentence.list.input <- reactive({get.sentences.term(sentence.list.f(),
                              term.vector())})
  output$sentence.list.input <- renderDataTable(sentence.list.input)
}
